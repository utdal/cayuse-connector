export default {
    template: '#user_affiliation_search_template',
    ...
    methods: {
        ...
        searchUserAffiliations() {
            this.user_searching = true;
            this.user_search_performed = false;
            this.user_search_results = [];
            fetch(`${this.user_affiliation_search_url}?${new URLSearchParams({user_query:this.user_query})}`)
                .then(response => {
                    if (!response.ok) throw Error(response.statusText);
                    return response.json();
                })
                .then(data => {
                    this.user_search_results = data?.['internal-affiliations'] ?? [];
